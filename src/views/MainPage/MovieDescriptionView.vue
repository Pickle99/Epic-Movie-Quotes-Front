<template>
  ...
  <div class="md:ml-96 text-white">
    <h1 class="mt-28 font-bold hidden md:block">{{$t('message.movie_description')}}</h1>
    <div class="mt-8 flex flex-col md:flex-row mx-5 md:mx-0">
      ...
      <div class="ml-5">
        <div class="flex items-center justify-between">
          ...
          <div v-if="movie.user_id == userId" class="mt-5 md:mt-0 flex px-7 py-3 rounded-md bg-[#24222F] ">
  ...
  <div class="text-white md:ml-96 mt-8 flex items-center">
    <p :class="movie.user_id == userId ? 'md:block hidden border-r-2 pr-5 py-1 border-gray-800 mr-5' : 'md:block hidden pr-5 py-1 mr-5'">{{$t('message.quotes')}} ({{movie.quotes.length}})</p>
    <RouterLink
v-if="movie.user_id == userId"
    ...
    >
      ...
      <div class="md:hidden border-b-2 border-gray-700 mt-5 mx-7 flex justify-center"></div>
    <div class="text-white my-7 ml-7 md:hidden">
      <p class="font-bold">
        {{$t('message.all_quotes')}}
      </p>
      <p>
        ({{$t('message.total')}} {{movie.quotes.length}})
      </p>
    </div>
